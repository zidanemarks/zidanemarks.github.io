...
`class thread_dt`是一个容器类.所包含的五个部分描述了计算机任务线程执行过程所必须上下文信息，通过这些信息，处理器可以跳转到指定sub-routine执行任务，一般情况下，在现场保存时，只需保存GPR寄存器文件的两类寄存器，**Saving Rigisters**与**Temporry Register**
