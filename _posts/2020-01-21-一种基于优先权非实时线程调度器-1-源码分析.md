...
graph TD
    id1[调度器单例初始化]-->
	id2[extcontext数组初始化]-->id3[high ready_queue 初始化]-->id6[spare queue初始化]
	id2-->id4[normal ready_queue初始化]-->id6
	id2-->id5[low ready_queue初始化]-->id6-->id7[逐一挂载extcontext成员到availablecontext数组]
	id7-->id8[把available中的可用线程对象加入到spare_queue中]
```
