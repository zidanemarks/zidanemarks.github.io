...
**调度器**(**scheduler**)是计算机操作系统内核对进程/线程分配CPU计算资源的重要模块

![调度器功能](https://github.com/zidanemarks/zidanemarks.github.io/tree/master/img/post-sheduler-structure-web1.jpg)
...
**本篇文章为第一部分主要分析调度器的源码实现。** 在低级调度器中，一般划分为：**非抢占式**和**抢占式**。非抢占式的调度器中，一个进程/线程要不然就一直执行到底，要不然就自己主动资源放弃处理器，以处理I/O请求，调度器只是安排先后顺序；而抢占式调度器里，调度器从当前线程手中把处理器抢走，交给另一个进程/线程，有这么一个主动的动作。不管是哪一种，调度器的执行步骤一般如下：

- 获得处理器的控制权
- 保存当前正在运行的进程/线程状态（PCB，process control block）
- 选择一个新的进程/线程来执行
- 把新选择出来的进程/线程分发给处理器运行，将第二步中的状态再加载到CPU寄存器中
   
在调度器调度过程中，进程/线程状态非常重要，其包括很多程序执行的关键信息：当前程序执行位置（PC指针），过程调用（sub-routine）返回地址，程序在存储空间的现场（**contex**，程序上下文）等等。

以下内容将就上述要点具体分析调度器的实现结构和工作流程
...
![调度器结构关系](https://github.com/zidanemarks/zidanemarks.github.io/tree/master/img/post-bg-threadlib-web.jpg)
...
`class thread_dt`是一个容器类.所包含的五个部分描述了计算机任务线程执行过程所必须上下文信息，通过这些信息，处理器可以跳转到指定sub-routine执行任务，一般情况下，在现场保存时，只需保存GPR寄存器文件的两类寄存器，**Saving Registers**与**Temporry Registers**

![microbalze寄存器功能描述](https://github.com/zidanemarks/zidanemarks.github.io/tree/master/img/post-bg-mb_registers_web.jpg)
